...
<a href="${basePath }/fore/api_detail/consLove.do?conNum=${conMain.orderNum  }"  class="on">爱情运势</a>
</div>
...
<dt><span><img src="${basePath }/fore_source/image/constell/${conMain.orderNum }.png"  alt="${conMain.conName  }"></span></dt>
	...
	<dd><h4>${conMain.conName  }2017年爱情运势</h4>

	<ul>
	<li><label>整体运势：</label>
	<c:forEach var="i" begin="1" end="${conLove.zhengti }" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
	</li>
	<li><label>女生指数：</label>
	<c:forEach var="i" begin="1" end="${conLove.nvsheng}" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
	</li>
	<li><label>男生指数：</label>
	<c:forEach var="i" begin="1" end="${conLove.nansheng }" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
	</li>
	<li class="desc"><label>短评：</label>${conLove.duanping }</li></ul></dd>
...
<p><strong class="p1">综合概述</strong><span>${conLove.zongheb }</span></p>
<p><strong class="p9">女生</strong><span>${conLove.nvshengb }</span></p>
<p><strong class="p8">男生</strong><span>${conLove.nanshengb }</span></p><div class="z"></div></div>
</div>
</div>
</div>
