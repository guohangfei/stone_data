...
<div class="top"><strong>${conMain.conName }</strong><!-- 星座名 -->
<a href="${basePath }/fore/api_detail/consToday.do?conNum=${conMain.orderNum }" class="on">今日运势</a>
...
<a href="${basePath }/fore/api_detail/consMonth.do?conNum=${conMain.orderNum }">本月运势</a>
...
<dt><span><img src="${basePath }/fore_source/image/constell/${conMain.orderNum }.png"  alt="${conMain.conName }"></span></dt>
	...
	<dd><h4>${conMain.conName  }今日运势</h4>
	<ul>
		<li><label>整体运势：</label>
		<c:forEach var="i" begin="1" end="${conDay.zhengti }" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
		</li>	
		<li><label>爱情运势：</label>
		<c:forEach var="i" begin="1" end="${conDay.aiqing}" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
		</li>
		<li><label>事业学业：</label>
		<c:forEach var="i" begin="1" end="${conDay.shiye }" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
		</li>
		<li><label>财富运势：</label>
		<c:forEach var="i" begin="1" end="${conDay.caifu }" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
		</li>
		
		<li><label>健康指数：</label>${conDay.jiankang }</li>
		<li><label>商谈指数：</label>${conDay.shangtan }</li>
		<li><label>幸运颜色：</label>${conDay.yanse }</li>
		<li><label>幸运数字：</label>${conDay.shuzi }</li>
		<li><label>速配星座：</label>${conDay.supei }</li>
		<li class="desc"><label>短评：</label>${conDay.duanping }</li>
...
<p><strong class="p1">整体运势</strong><span>${conDay.zhengtib }</span></p>
<p><strong class="p2">爱情运势</strong><span>${conDay.aiqingb }</span></p>
<p><strong class="p3">事业学业</strong><span>${conDay.shiyeb }</span></p>
<p><strong class="p4">财富运势</strong><span>${conDay.caifub }</span></p>
<p><strong class="p5">健康运势</strong><span>${conDay.jiankangb }</span></p>

<div class="z"></div></div><!-- <script type="text/javascript" src="js/highcharts.js"></script> -->
