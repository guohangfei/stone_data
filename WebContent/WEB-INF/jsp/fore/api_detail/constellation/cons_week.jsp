...
<a href="${basePath }/fore/api_detail/consWeek.do?conNum=${conMain.orderNum  }" class="on">本周运势</a>
...
<dt><span><img src="${basePath }/fore_source/image/constell/${conMain.orderNum }.png" alt="${conMain.conName }"></span></dt>
	...
	<dd><h4>${conMain.conName  }本周运势<small></small></h4>

	<ul>
	<li><label>整体运势：</label>
	<c:forEach var="i" begin="1" end="${conWeek.zhengti }" step="1">   
		<img src="${basePath }/fore_source/image/constell/star.gif" />     
	</c:forEach>
	</li>
	<li><label>爱情运势：</label>
	<c:forEach var="i" begin="1" end="${conWeek.aiqing}" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
	</li>
	<li><label>事业学业：</label>
	<c:forEach var="i" begin="1" end="${conWeek.shiye }" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
	</li>
	<li><label>财富运势：</label>
	<c:forEach var="i" begin="1" end="${conWeek.caifu }" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
	</li>
	<li><label>健康运势：</label>
	<c:forEach var="i" begin="1" end="${conWeek.jiankang }" step="1">   
			<img src="${basePath }/fore_source/image/constell/star.gif" />     
		</c:forEach>
	</li>
	<li><label>幸运颜色：</label>${conWeek.yanse }</li>
	<li><label>幸运星座：</label>${conWeek.xingzuo }</li>
	<li><label>提防星座：</label>${conWeek.difang }</li>
	<li class="desc"><label>短评：</label>${conWeek.duanping }</li>
	</ul></dd>
...
<p><strong class="p1">整体运势</strong><span>${conWeek.zhengtib}</span></p>
<p><strong class="p2">爱情运势</strong><span>${conWeek.aiqingb }</span></p>
<p><strong class="p3">事业学业</strong><span>${conWeek.shiyeb }</span></p>
<p><strong class="p4">财富运势</strong><span>${conWeek.caifub }</span></p>
<p><strong class="p5">健康运势</strong><span>${conWeek.jiankangb }</span></p><div class="z"></div></div>
