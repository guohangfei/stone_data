...
(function ($) {
    ...
    var methods = {
                ...
                if (options.scroll) {
                    ...
                    if (options.isOverflown) {
                        ...
                        scrollContainer.animate({ scrollTop: destination }, 1100, function () {
                            if (options.focusFirstField) { try { first_err.focus(); } catch (e) { } }
                        });
                    } else {
            ...
            for (var i = 0; i < rules.length; i++) {
                ...
                switch (rules[i]) {
                    ...
                    case "creditCard":
                        ...
                        break;
                    default:
                        errorMsg = methods._other(field, rules, i, options);
        ...
        _required: function (field, rules, i, options) {
            switch (field.prop("type")) {
                ...
                case "select-one":
                    ...
                    if ((!field.val()) || field.val() == "-1")//add -1 xss 20140910
		...
		*/
        ...
        /**
      * Check the minimum bytes size (in characters)
      *
      * @param {jqObject} field
      * @param {Array[String]} rules
      * @param {int} i rules index
      ...
      */
        _maxByteSize: function (field, rules, i, options) {
            ...
            var byteLen = 0;
            for (var i = 0; i < len; i++) {
                byteLen += field.val().charCodeAt(i) > 255 ? 2 : 1;
            }
       ...
       */
		...
		*/
        ...
        //其它验证规则通用方法
        _other: function (field, rules, i, options) {
            try {
                if (field.val() != "") {
                    var errorSelector = rules[i];
            ...
            if (!options.isError) {
                $.ajax({
                    ...
                    success: function (json) {
                        ...
                        var errorFieldId = json.jsonValidateReturn[0];//改动地方
                        //var errorField = $($("#" + errorFieldId)[0]);
                        ...
                        if (errorField.length == 1) {
                            //var status = json[1];
                            var status = json.jsonValidateReturn[2];//改动地方
                            // read the optional msg from the server
                            //var msg = json[2]; 
                            var msg = json.jsonValidateReturn[1];//改动地方
                            if (!status) {
        ...
        _calculatePosition: function (field, promptElmt, options) {
            ...
            if (!methods.isRTL(field)) {
                switch (positionType) {
                    ...
                    case "topRight":
                        if (overflow)
                            promptleftPosition += fieldWidth - 100;
                        else {
                            promptleftPosition += fieldWidth - 100;
                ...
                switch (positionType) {
                    ...
                    case "topRight":
                        if (overflow)
                            promptleftPosition = -promptElmt.width() - 60;
                        else {
                            promptleftPosition += fieldWidth - promptElmt.width() - 60;
                            promptTopPosition += -promptHeight + 5;
                        }
                        break;
                    case "centerRight":
